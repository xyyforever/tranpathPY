...
def index(request):
    ...
    if "D" + purCodeMain in purviewList:
        purviewD = "T"
    return render(request, "basicInfo/dicWareclass/index.html", locals())


# 获得tree数据
from basicInfo.helper import queryset_to_list
def getWareclassTreeList(request):
    dic_areas = basicinfo_models.DicWareclass.objects.filter(deletetime=None).order_by("sorts")
...
def ajaxMainMenuJson(request):
    mainMenu = basicinfo_models.DicWareclass.objects.filter(deletetime=None).order_by("sorts")
    newList = []
    if mainMenu:
...
# 保存
@csrf_exempt
def ajaxSaveDicWareclass(request):
    data = request.POST
    scription = ""
    print(data)
    try:
        with transaction.atomic():
            if data != "":
                ...
                if id == "":  # 添加
                    # 判断同名区域是否存在
                    model = basicinfo_models.DicWareclass.objects.filter(name=name, deletetime=None)
                    if model:
                        scription = "区域名%s已经存在" % name
                        return JsonResponse({"result": "F", "scription": scription, "extendInfo": "你的extendInfo1"})

                    try:
                        model = basicinfo_models.DicWareclass.objects.filter(parentId=parentid, deletetime=None).latest(
                            "sorts")
                        if model:
                            sorts = model.sorts + 1
                        else:
                            sorts = 1
                    except basicinfo_models.DicWareclass.DoesNotExist:
                        sorts = 1

                    new_area = basicinfo_models.DicWareclass.objects.create()
                    new_area.name = name
                    ...
                    basicinfo_models.DicWareclass.objects.filter(id=id).update(name=name, parentId=parentid, memo=memo,
...
# 删除
@csrf_exempt
def ajaxDelDicWareclass(request):
    try:
        with transaction.atomic():
            ...
            if idsForDelete != "":
                ...
                basicinfo_models.DicWareclass.objects.filter(id__in=idsList).update(
                    deletetime=datetime.now(), deleter=system_models.User.objects.filter(id=deleter)[0])
                basicinfo_models.DicWareclass.objects.filter(parentId__in=idsList).update(
                    deletetime=datetime.now(), deleter=system_models.User.objects.filter(id=deleter)[0])
